...
suffic 'Z', the representation is binary, otherwise the representation
is decimal.
...
type            = NamedType | ArrayType | StructUnionType | PointerType 
                  | ProcedureType | MethodType | ObjectType | InterfaceType
....
...
NOTE: Each instance of an object carries explicit class information, whether the instance
    is on the stack or on the heap. The runtime is responsible to assure that each instance
    is properly initialized with this information before it is used.
...
An interface type declares a set of methods
without providing an implementation. A method of an interface is
declared as an ABSTRACT procedure bound to the interface type (see
<<procedures>>); the receiver parameter is explicit, the first parameter
in the parameter list, and of the interface type itself.

Variables of an interface type have an
_interface reference_ as value. An interface reference is represented by
an _ifaceref_ value in the runtime, an opaque compound of two _intptr_:
one pointing to a record or object instance, the other to an internal
procedure table which provides one implementation procedure for each
method declared in it. 

An interface reference is created with the
_ldiface_ instruction from a record or object pointer. The referenced
type is said to _implement_ the interface; for each abstract method it must  
declare a bound procedure with the same name whose formal
parameter list and return type match the method's declaration (not
regarding the receiver parameter, which is a pointer to the implementing
type). For a record, the procedure table is selected based on the static
type of the operand; for an object, it is selected based on the dynamic
type. The instance _intptr_ of the resulting interface reference is the
operand pointer itself.

A method is invoked by loading a method
reference from the interface reference with _ldmeth_, which combines the
instance _intptr_ with the corresponding entry of the procedure table,
and calling it with _callmi_. 

NOTE: no dispatch happens at the call; the implementation procedure was 
determined when the interface reference was created with _ldiface_.

A variable or parameter of interface type
may assume the value NIL, in which case both _intptr_ of the compound
are null. A NIL interface reference is created with the _ldnull_ipp_
instruction. Two interface references can be compared with _ceq_; they
are equal if both _intptr_ of one value are equal to the corresponding
_intptr_ of the other value.
...
ExpInstr = 'add' | 'and' | ('call' | 'calli') qualident | 'callmi' qortrident
    ...
    | 'div' | 'div_un' | 'dup' | 'getreg' unsigned
    | 'initobj' qualident | 'isinst' qualident
    ...
    | ('newarr'|'newarr0'|'newarrgc'|'newvla'|'newobj'|'newobj0'|'newobjgc') qualident 
...
Constructors are used to declare and initialize struct, union, array, and pointer literals. 

.Syntax of textual representation:
....
constructor = NamedType component_list
component_list = '{' [ component {[','] component} ] '}'
component = [(ident|unsigned) '='] ( ConstExpression | component_list )
....

A constructor consists of an explicit `NamedType` prefix followed by a list of components. Components can be **designated** (explicitly prefixed with a field name or array index) or **anonymous** (providing only the value). 

Before any components are applied, the memory for the literal is **fully initialized with zeros**. Because of this, it is not required to provide a component for every field or element; any omitted fields or elements will default to zero.

Anonymous and designated components can be freely mixed within the same constructor. The compiler resolves which field or element a component applies to based on the following rules:

* A designated component explicitly initializes the named field or indexed element.
* An anonymous component initializes the "current" field or element. At the start of the list, the "current" position is the first field or the 0th element. 
* Whenever a component (designated or anonymous) is applied, the "current" position advances to the immediately following field or index.
* Struct types: anonymous components map to the struct's fields in their exact order of declaration. A named component (`ident = value`) initializes that specific field, and any subsequent anonymous components will initialize the fields declared immediately after it.
* Array types: anonymous components map to array elements sequentially. An indexed component (`unsigned = value`) initializes the element at that specific integer index, and subsequent anonymous components will continue sequentially from `index + 1`. If the array type is an open (unsized) array, its final length is determined by the highest initialized index plus one.
* Union types: a constructor for a union must contain exactly one component. A named component must be used to explicitly identify which option of the union is being initialized.
* Pointer types: A constructor for a pointer must contain exactly one anonymous component, which must be an unsigned integer constant representing the memory address.

For any field or element that is itself of a struct, union, array, or pointer type, a nested constructor is required. Because the exact type of the field or element is already known from the parent's type definition, the `NamedType` prefix can be omitted for the inner constructor, allowing the use of a bare `component_list` (`{ ... }`).
...
The _function_ reference either points to a
...
|TBD 
|cast_i4 
|Bitwise reinterpretation of _value_ on stack as int32.

|TBD 
|cast_i8 
|Bitwise reinterpretation of _value_  on stack as int64.

|TBD 
|cast_r4 
|Bitwise reinterpretation of _value_, on stack as float32.

|TBD 
|cast_r8 
|Bitwise reinterpretation of _value_,  on stack as float64.
...
=== *castptr – cast to a pointer type*
...
|74 <__T__> 
|castptr _type_ 
|Cast value to pointer to _type_
|===

`…, value -> …, ptr`

The castptr instruction reinterprets the
value on the stack as a pointer to _type_.
The _type_ references the base type of the pointer.

The acceptable value types and their corresponding 
result data types are encapsulated as follows:

.castptr supported input and output types
[#castptr]
|===
|*Input Type* |int32  |int64 |intptr |F
|*Result Type*  |intptr |intptr|intptr |x
|===

If _value_ is 0 or null, castptr succeeds and
returns null.

On a 64 bit architecture, an int32 value is zero extended to 64 bits.
On a 32 bit architecture, an int64 value is truncated to 32 bits.

// TODO: originally castptr only supported pointer to pointer casts; I had to 
// add integer to pointer casts because of the C frontend; maybe I should add
// and option to the validator whether it accepts integers or not?
...
|===
|                             |*Input*    |           |           | 
|*Convert-To*                 |*int32*    |*int64*    |*intptr*   |*F* 
|*int8, uint8, int16, uint16* |Truncate^1^|Truncate^1^|Nop        |Truncate to zero^2^ 
|*int32, uint32*              |Nop        |Truncate^1^|Nop        |Truncate to zero^2^
|*int64*                      |Sign extend|Nop        |Nop        |Truncate to zero^2^
...
=== *getreg – push a register value onto the stack*
...
|25 
|getreg _regwidth_
|Push the value of the referenced register onto the stack
|===

`…, -> …, value`

The getreg instruction pushes the value of the referenced register to the stack. 
The register is identified by a plain numeric index whose meaning is not defined here. 
Therefore, the register number is accompanied by a second value that specifies the register width in bytes, 
where 0 denotes the platform-dependent pointer width. Both values are encoded into a single integer (_regwidth_): 
the lower 16 bits contain the register number, and the upper 16 bits contain the width.
...
=== *isinst – test if an object is an instance of a class*
...
`…, -> …, address of argument number _argNum_`
...
reference (_ifaceref_) onto the stack. The _interface_ reference points
to an interface declaration. _obj_ can be a record or object pointer.
The method table is chosen based on the static type (in case of a record)
or the dynamic type (in case of an object) of _obj_.
...
=== *ldnull – load a null pointer or reference*
...
|TBD 
|ldnull_ipp
|Push a null methref or ifaceref on the stack.

|===

`… -> …, null value`

The ldnull instruction pushes a null pointer (type intptr) on the stack,
whereas ldnull_ipp pushes a compound of two null pointers on the stack.
...
|8D <__T__> 
|newarr _etype_ 
|Create a new array with elements of type _etype_.
...
_etype_. _numElems_ (of type unsigned int32) specifies the number of
elements in the array. Valid array indexes are 0 ≤ index < _numElems_.
The elements of an array can be any type. The elements are not initialized.

The newarr0 instruction does the same as newarr, with the addition that elements are initialized.

The newarrgc instruction does the same as newarr0, but uses managed memory automatically
garbage collected when the array is no longer referenced.

NOTE: This operation is only for open array types. Array types with fixes size are created with newobj
    instead. Even if elements are not initialized, object instances are still initialized with
    type information. This also applies to embedded object instances stored by value.
...
|EE 0F <__T__> 
|newvla _etype_ 
|Create a new VLA with elements of type _etype_.
|===
...
|73 <__T__> 
|newobj _type_ 
|Allocate an instance of a structured type.

|TBD 
|newobj0 _type_ 
|Allocate and initialize an instance of a structured type.
...
The newobj instruction creates a new instance of _type_. The fields are not initialized.

The newobj0 instruction does the same as newobj, with the addition that fields are initialized.

The newobjgc instruction does the same as newobj0, but uses managed memory automatically
garbage collected when the object is no longer referenced.

NOTE: This operation applies to all structured types of fixed size, including non-open arrays.
    Even if fields/elements are not initialized, object instances are still initialized with
    type information.
...
`_offset_ * sizeof(_basetype_)`. Offset can be of int32 or int64
type.
...
implementation chooses to add. Specifically, array elements lie sizeof bytes apart.

NOTE: this instruction is required when e.g. a struct is serialized or deserialized 
to a byte stream.
...
Frontends shall provide local variable names, even if the source language doesn't require them;
...
been compiled earier by the same compiler. 
A FOREIGN procedure uses the C calling convention.
FOREIGN may be followed by an explicit name or address of the procedure implementation.
If neither is given, the backend’s implementation-defined naming scheme is used.
The name shall be specified either as a string literal or as a qualident denoting a constant 
declaration whose value is a string literal. The characters of the string literal, excluding the 
quotation marks, shall form a valid external name accepted by the target toolchain.
The address shall be specified either as an unsigned integer literal or as a qualident denoting a 
constant declaration whose value is an unsigned integer literal.
...
procedure can be declared INIT or ENTRY, in which case it is automatically called
at module load time. A procedure declared with ENTRY has the meaning of
"INLINE INIT"; either an INIT or ENTRY procedure can be declared.
...
Procedures may be associated with an object, struct, or interface
...
A procedure declared ABSTRACT has no body; abstract procedures are
used to declare the methods of an interface type. The binding of an
abstract procedure names an interface type, and the receiver
parameter is of the interface type itself.

.Syntax of textual representation:
....
ProcedureDeclaration = ProcedureHeading 
                         (['INLINE'|'INVAR'|'INIT'|'ENTRY'] [';'] ProcedureBody 
                          | [';'] EXTERN
                          | [';'] FOREIGN [string|unsigned|qualident]
                          | [';'] FORWARD
                          | [';'] ABSTRACT ) 
...
Frontends shall provide parameter names, even if the source language allows to omit them.
Parameter names must be unique within the formal parameter list.
...
1. The evaluation stack must be entirely empty upon entry to a procedure.
2. When the ret statement is executed, the stack must be exactly empty for a procedure with no return type, or contain exactly one value of the correct return type for a function procedure.
3. For compound statements (if, loop, while, etc.) and labels, the depth and types of the evaluation stack must be identical across all control-flow paths that reach them (Stack Consistency).
...
SimpleStatement = 'cli' | 'free'| 'exit' | 'goto' ident
    | 'label' ident | 'pop' | 'putreg' unsigned | 'ret'
    ...
    | 'stfld' trident | 'sti' | 'stind' qualident
...
|TBD 
|cli 
|Clear interrupts: unconditionally disables interrupts
|===

`…, -> …`

The cli instruction unconditionally disables maskable hardware interrupts.
...
allocated with `newobj`, `newobj0`, `newarr` or `newarr0` expressions.
Applying `free` on memory allocated with `newobjgc` or `newarrgc` is undefined.
...
=== *putreg – pop a register value from the stack*
...
|25 
|putreg _regwidth_
|Pop the value from the stack to the referenced register
|===

`…, value -> …,`

The putreg instruction sets the value of the referenced register by popping it from the stack. 
The register is identified by a plain numeric index whose meaning is not defined here. 
Therefore, the register number is accompanied by a second value that specifies the register width in bytes, 
where 0 denotes the platform-dependent pointer width. Both values are encoded into a single integer (_regwidth_): 
the lower 16 bits contain the register number, and the upper 16 bits contain the width.
...
|TBD 
|sti 
|Set interrupts: unconditionally enables interrupts.
|===

`…, -> …`

The sti instruction unconditionally enables maskable hardware interrupts.
