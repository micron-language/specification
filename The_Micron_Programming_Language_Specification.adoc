...
- Oberon uses _floor division_, while Micron uses _truncating division_ (as most programming
  languages and integer division instructions of common processors)
...
upper- or all lower-case letters and cannot be used as identifiers. All
words listed below are reserved (only upper-case versions shown).
...
NOTE: Lower- and upper-case versions of keywords and built-in types and procedures can even be
  mixed in the same module; so even if only the lower-case versions would be used, the upper-case
  versions are still reserved.
...
be followed by an export mark ("\*", "-" or "~") in its declaration to indicate that it is exported. 
An identifier _x_ exported by a module _M_ may be used in other modules, if they import _M_ (see <<Modules>>). 
Such identifiers are called _public_.
The identifier is then denoted as _M.x_ in these modules and is called a _qualified identifier_. 
Identifiers marked with "-" in their declaration are _readonly_ in importing modules.
Identifiers without an export mark, or marked with "~" in their declaration, are _not_ exported and considered _private_ to the module.
...
NOTE: Whether an identifier is _public_, _readonly_ or _private_ is called its _visibility_. The default visibility of an identifier
  can be changed from _private_ to something else using the visibility module attribute (see <<module-attributes>>). 
...
The expression can be of basic type, or a string liteal or hex string (see <<hex-string-literals>>).
...
predeclared or INVAR functions (see <<predeclared-procedure-reference>>, <<procedure-declarations>>) or
...
this `MAX(INT32)+1I8` has to be used.
...
|UINT8, U1, BYTE |1 byte |the integers between 0 and 255
|INT8, I1 |1 byte |the integers between -128 and 127
|INT16, I2, SHORTINT |2 byte |the integers between -32'768 and 32'767
|UINT16, U2 |2 byte |the integers between 0 and 65'535
|INT32, I4, INTEGER |4 byte |the integers between -2'147'483'648 and 2'147'483'647
|UINT32, U4 |4 byte |the integers between 0 and 4'294'967'295
|INT64, I8, LONGINT |8 byte |the integers between -9'223'372'036'854'775'808 and
9'223'372'036'854'775'807
|UINT64, U8 |8 byte |the integers between 0 and 18'446'744'073'709'551'615
|FLT32, R4, REAL |32 bit |an IEEE 754 floating point number
|FLT64, R8, LONGREAL |64 bit |an IEEE 754 floating point number
...
length is an unsigned integer. The elements of the array are designated
...
be accepted by a compiler, for fixed and dynamic lengths.

[[record-types]]
==== Record types

A record type is a composite structure consisting of a fixed number of fields, 
which may be of different types. The declaration specifies the name and type of each field. 
Field identifiers are in scope from their point of declaration to the end of the record definition, 
and are accessible via designators on record variables (see <<Operands>>). If a record type is exported 
from a module, its fields remain private by default; fields intended to be visible outside the declaring 
module must be explicitly marked as public.

A record type may include an optional variant part, introduced by the CASE keyword. 
This allows variables of the same record type to contain different sets of fields depending on the active variant. 
The variants are discriminated by a specific tag field. Because fields in different variants share the same memory storage, 
the total size of the variant part is equal to the size of its largest case. No field within a variant part may be of an 
object type (see <<object-types>>).

The fields, variants and methods (see <<type-bound-procedures>>) of a record
...
Fields or variants of record or object type of the fixed part or
the variant part can be prefixed by the INLINE (or
...
Any field or variant of a record or object type, whether in the fixed or variant part, 
may be prefixed with the INLINE (or IN) keyword. Doing so makes the field's name transparent, 
promoting its inner members so they are directly embedded and accessible within the enclosing record.
...
All direct members (fields, variants, or methods) of a record must have unique names. 
If an INLINE field or variant contains a promoted member with the same name as a direct member, 
the direct member takes precedence, and the promoted member must be accessed via its explicit field designator. 
If multiple INLINE fields or variants contain members with the same name, an unqualified reference resolves to 
...
In the case of a pointer to a self-referential array declaration, the following 
approach can be used (use the extra ItemDesc2Array declaration so the 
pointer has not to be resolved within the ItemDesc2 declaration): +
`ItemDesc2 = RECORD value: INTEGER; children: POINTER TO ItemDesc2Array END` +
`ItemDesc2Array = ARRAY OF ItemDesc2`
...
A designator of the form `v(T)` constitutes a *type guard*. It is applicable only if the static type of `v` is a pointer to an object type, and `T` is also a pointer to an object type. Furthermore, `T` must be an extension of the static type of `v`. The form `v(@O)` is also supported to convert the object type `O` to a pointer type in place.

The type guard asserts that the dynamic type of the object referenced by `v` is equal to the object base type of `T` (or an extension thereof). If `v` is `NIL` or the assertion fails, program execution aborts. 

If the assertion succeeds, the designator yields the value of `v` treated statically as type `T`. This allows access to the extended fields and type-bound procedures of `T`.

NOTE: Type guards are strictly checked at run-time. For unchecked, static reinterpretation of memory addresses, such as casting pointers to records or basic types, use the `CAST` predeclared procedure instead (see <<predeclared-function-procedures>>). 
...
When used as a binary operator, both operands must be of either signed integer, unsigned integer or
floating point type.
...
As an exception, if one operand is a signed or unsigned integer and the other is a _universal integer_ type literal, 
the latter is treated as the smallest signed or unsigned integer type that can represent its value compatible with the former.
If the value is too large for the largest supported signed integer type, an error is reported.
...
`DIV` yields the quotient rounded towards zero (truncating division), 
and `MOD` yields the corresponding remainder. They are related by the following formulas, 
defined for any `x` and any `y # 0`:
...
The result of `x MOD y` is either zero or has the sign of `x`. 


NOTE: This deviates from Oberon, Pascal and Modula-2, which are defined by floor division, 
where the operations are related by `x = (x DIV y) * y + (x MOD y), 0 \<= (x MOD y) < y` instead
and the remainder is never negative
In Oberon, `(-7) DIV 2 = -4` and `(-7) MOD 2 = 1`, whereas in Micron `(-7) DIV 2 = -3` and `(-7) MOD 2 = -1`. 
The two definitions agree whenever both operands are non-negative. 
Micron implements the same semantics as the `/` and `%` operators of C and most other contemporary languages, 
and as the integer division instructions of common processors.
Use the predefined OBDIV() and OBMOD() functions if you need Oberon compatibility.
...
- all other operands of unsigned integer type, the result is the next wider signed integer type, or an error if the operand is of type UIN64;
- all other operands, the type of the result is the same as the type of the operand.

NOTE: the '+' and '-' unary operators can be used to convert unsigned to a signed integer values. 
This behaviour is a shortcut of the SIGL() predeclared function (see <<predeclared-function-procedures>>).
...
- T is an object or pointer to object type which is an _extension_ of the static 
  type of `v` (see <<definition-of-terms>>).
...
The address operator @ cannot be applied to a record field declared with a BITS specification.

Returning or escaping a pointer to a local variable results in Undefined Behavior. 
...
Constructors can be used to initialize variables of
record, object, array, pointer and (non type-bound) procedure type via assignments, or to create an
...
inferred (e.g. by the left side of an assignment or formal parameter),
the NamedType prefix can be left out.
...
When a constructor is a literal, NamedType may be an open array in which case the number
of elements is determined based on the components. 

When a constructor is prefixed with the
address-of operator ('@'), then it represents an anonymous, local variable.
...
of indizes. The size of an open array type is determined by the 
number of components or the largest index component.

If NamedType is an array of char, it can be initialized
with a single string literal. The string literal (including terminating
zero) must fit into the char array, or it determines its size if NamedType
is an open char array. 

If NamedType is an array of byte, it can be initialized
with a single hex string literal. The literal must exactly fit 
the size of the byte array, or it determines its size if NamedType
is an open byte array.

If NamedType is a pointer or procedure type, then there
is exactly one anonymous component which is an unsigned integer
constant representing the address. Such a constructor can also
be used in a designator instead of the qualident.

For each field or element which is of
record, object, array, pointer or procedure type, an embedded constructor is
required.

Any field/element not explicitly specified in a constructor assumes its DEFAULT value.
...
NOTE: A <<hex-string-literals>> (or a constant of hex string literal type) is 
  treated the same way as an array of byte constructor when it is used
  (i.e. assigned, passed or returned).
...
set to E; otherwise R is set to NIL. 
...
module ExceptionExample
  ...
  proc Print(const str: ^array of char)
  begin
    println(str^)
    raise(@ex1)
...
available. If EXTERN is followed by the ident C and/or a string, then the procedure is
...
procedure is implemented in Micron. Using the string
literal, the name of the procedure in the external library can be set,
...
Forward declarations are marked by '^'. For each forward declaration, there
must be exactly one procedure in the same scope with the same name, which is 
not a forward declaration (the implementation). The formal parameters and result type 
of a forward declaration and the corresponding implementation must match.
...
by the name "self". The procedure name at the end of the procedure
can be prefixed with the receiver type name.
...
Type-bound procedures cannot be INVAR or EXTERN.
...
The following attributes are defines so far:
...
This attribute is used to set the default visibility for declarations on module level.
By default, declarations without an explicit export mark are considered _private_. The 
expression can be "*" for _public_, "-" for _readonly_ or "~" for _private_; the corresponding identifiers
are not case sensitive. See also <<declarations-and-scope-rules>>.
...
Meta parameters can be constrained with a type, in which case this
type is assumed in the generic module.
For the types which are known at the point of meta parameter declaration,
which in general are only the <<basic-types>>, the constraint can be 
directly added to the declaration like in a normal parameter declaration.
For types, which need an import or type declaration within the generic module,
a WHERE declaration can be used thereafter. The correspondence between a WHERE
declaration and a meta parameter is established by name equivalence.
There can only be one type constraint per meta parameter. 

A type constraint of a meta parameter can in particular be an
interface type which is a special but legal use of interface types.

The type of
the actual meta parameter must be assignment compatible with the
constraint type (see <<definition-of-terms>>), which is
checked when the generic module is instantiated. 
...
Every distinct set of meta actuals creates a distinct module identity.
Two sets of meta actuals are equal if the corresponding type actuals denote 
the same type, and the corresponding const actuals have the same type and value.
See also <<Modules>>.

NOTE: In consequence, not every import declaration with meta actuals
creates a new module identity, but the compiler automatically unites all import
declarations with the same set of meta actuals.

unites all import declarations with the same set of meta actuals, .
...
When an interface type is used as a type constraint, then the type checker
...
the interface (see <<interface-types>>).

NOTE: This is a compile-time use of interfaces with no extra runtime cost. 
...
Signed integer types:: 
	INT8, INT16, INT32, INT64, SHORTINT, INTEGER, LONGINT, I1, I2, I4, I8

Unsigned integer types::
	BYTE, UINT8, UINT16, UINT32, UINT64, U1, U2, U4, U8

[[floating-point-types]]
Floating-point types::
	REAL, LONGREAL, FLT32, FLT64, R4, R8
...
- T~v~ is a non-open array of UINT8, T~e~ is
  a Hex string literal with the same size as the array;
...
| |CHAR |CHAR |BOOLEAN
| |CHAR array, string lit. |CHAR array, string lit. |BOOLEAN
...
|ASR(x,n) 
|x: signed integer, n: unsigned integer 
|signed integer 
|arithmetic shift right by n bits; the result type corresponds to the type of x; the operation is defined for n >= 0 and n < SIZE(x)*8; no overflow checks of n (may halt program)

|BAND(x,y) 
|x, y: integer 
|integer 
|bitwise AND; operand types and result type follow standard integer binary operator rules (see <<arithmetic-operators>>)

|BNOT(x) 
|x: integer 
|integer 
|bitwise NOT; result type corresponds to the type of x; a literal of _universal integer_ type has the smallest unsigned integer type that can represent its value

|BOR(x,y) 
|x, y: integer 
|integer 
|bitwise OR; operand types and result type follow standard integer binary operator rules (see <<arithmetic-operators>>)
...
|x, y: integer 
|integer
|bitwise XOR; operand types and result type follow standard integer binary operator rules (see <<arithmetic-operators>>)
...
|CAST(T,x)
|T,x: numeric types; T is a type name
|T 
|convert x to T; bitwise reinterpretation of x; SIZE(T) must equal SIZE(x)

|
|T,x: pointer types; T is a type name and can be prefixed with @ to create a pointer of T in place
|T 
|returns the pointer x statically retyped to the pointer type T; no run-time check is performed.
...
|v: non-open array 
|UINT32 
|length of array v; this is a pure compile-time operation
...
|x, y: signed integer
|signed integer 
|integer _floor division_ as implemented in Oberon, related by `x = OBDIV(x, y) * y + OBMOD(x, y), 0 \<= OBMOD(x, y) < y`. Operand types and result type follow standard integer binary operator rules (see <<arithmetic-operators>>)

|OBMOD(x, y) 
|x, y: signed integer
|signed integer 
|integer modulo operation based on the Oberon _floor division_. Operand types and result type follow standard integer binary operator rules (see <<arithmetic-operators>>)
...
| 
|x: pointer or procedure type 
|unsigned integer 
|a numeric representation of the pointer, the exact type of it being target dependent

|PTROFF(p, n) 
|p: pointer to T; n: integer type
|pointer to T
|returns an address corresponding to p plus n times size of T, where T is not ANY

|SIGC(x) 
|x: unsigned integer
|signed integer 
|bitwise reinterpret x as a signed integer of the same byte width; this is a shortcut of CAST(T,x)

|SIGL(x) 
|x: unsigned integer
|signed integer type T
|convert x to the next larger signed integer type, preserving its value; this is a semantic conversion equivalent to VAL(T,x) or the unary \+/- operator; reports an error if x is of type UINT64

|SHL(x,n) 
|x: integer, n: unsigned integer 
|unsigned integer 
|logical shift left by n bits; result type is the unsigned integer type corresponding to the size of x; the operation is defined for n >= 0 and n < SIZE(x)*8; no overflow checks of n (may halt program)

|SHR(x,n) 
|x: integer, n: unsigned integer 
|unsigned integer 
|logical shift right by n bits; result type is the unsigned integer type corresponding to the size of x; the operation is defined for n >= 0 and n < SIZE(x)*8; no overflow checks of n (may halt program)

|SIZE(T) 
|T: all types besides open arrays and string literals 
|UINT32 
|number of bytes required by T; compile time operation only for basic types

|STRLEN(s) 
|s: array of char, pointer to char, or string literal 
|UINT32 
|dynamic length of the string up to and not including the terminating 0X; compile time operation only for string literals

|USIG(x) 
|x: signed integer
|unsigned integer 
|bitwise reinterpret x as an unsigned integer of the same byte width; this is a shortcut of CAST(T,x)
...
|CLI()
|-
|Clear interrupts: unconditionally disables maskable hardware interrupts; available on level 0 and 1.

|COPY(d,s,n)
|d: pointer to array of char; s: pointer to array of char or string literal; n: integer
|Copies up to n-1 characters from s^ to d^ and guarantees d^ is 0X terminated. If s is longer than n-1, it truncates. If n <= 0, it does nothing.
...
| 
|pointer to T 
|decrement address by size of T, where T is not ANY
...
| 
|v: pointer to T; n: integer type
|decrement address by n times size of T, where T is not ANY

|DISPOSE(p) 
|p: pointer 
|free the memory allocated by NEW() and NEWINIT(); available from level 3.
...
|GETREG(r,v)
|r, v: unsigned integer type; r is a constant expression
|copy register r to value v; available on level 0 and 1
...
| 
|pointer to T 
|increment address by size of T, where T is not ANY
...
| 
|v: pointer to T; n: integer type
|increment address by n times size of T, where T is not ANY
...
|NEW(p) 
|p: pointer to T 
|allocate p^ of type T, uninitialized; available from level 3.

|NEW(p,n) 
|p: pointer to open array, n: integer type
|allocate p^ with length n, uninitialized; available from level 3.

|NEWGC(p) 
|p: pointer to T 
|allocate p^ of type T, initialized with default values, only available on level 5.

|NEWGC(p,n) 
|p: pointer to open array, n: integer type
|allocate p^ with length n, initialized with default values, only available on level 5.

|NEWINIT(p) 
|p: pointer to T 
|allocate p^ of type T, initialized with default values; available from level 3.

|NEWINIT(p,n) 
|p: pointer to open array, n: integer type
|allocate p^ with length n, initialized with default values; available from level 3.

|NOP() 
|-
|Call MIL NOP operation; needed for timing loops or pipeline flushing in very low-level drivers; available on level 0 and 1.
...
|PRINT(a) 
|a: basic type, string, char array, or pointer to char array
...
|PUTREG(r,v)
|r, v: unsigned integer type; r is a constant expression
|put value v to register r; available on level 0 and 1
...
|STI()
|-
|Set interrupts: unconditionally enables maskable hardware interrupts; available on level 0 and 1.
...
|*Bootstrap* 
|Hardware Init 
|Inlined code, `PUTREG`/`GETREG`/`CLI`/`STI`. No Stack. 
...
be set via a module attribute (see <<module-attributes>>) per module. 
A *default language level 3* is assumed if no explicit level is set.

A module of a given language level can only import modules of the same or lower language levels.

[NOTE]
====
In Micron (as in Oberon), when Module A imports Module B:

- Module B's body (BEGIN ... END) is executed first.
- Module A's body is executed second.
- The initialization order corresponds to the import order.

This assures that lower-level modules are effectively initialized before the higher-level modules.
A module defined at a specific safety or abstraction level cannot depend on a module that requires 
a more complex runtime environment. For example, a Systems module (Level 1) cannot import a Managed 
module (Level 5) because the Systems module does not guarantee the presence of a garbage collector.
Sometimes the Low Level needs to call the High Level (e.g. an interrupt handler in Level 1 needs 
to wake up a thread in Level 5). Since Level 1 cannot import Level 5, we define procedure types
and let higher-level modules register their procedures for callback by lower-level modules. 
====
...
procedures are implicitly INLINE and optionally INVAR. 
EXTERN is not supported. Recursion is not supported. NEW and DISPOSE are not
supported. PCALL, RAISE and Interfaces are not supported. 
...
Level 1 in addition has EXTERN procedures.
...
Level 2 extends level 1 with interfaces (static dispatch), and FINALLY sections.
...
and optional exceptions (PCALL, RAISE).
