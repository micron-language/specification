...
|x, n: unsigned integer 
|UINT32 or UINT64 
|logical shift right by n bits; result is UINT64 if x is UINT64, else UINT32; the operation is defined for n >= 0 and n < SIZE(x)*8; no overflow checks (may halt program)
