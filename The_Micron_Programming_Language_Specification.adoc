...
|INT32 or INT64 
|arithmetic shift right by n bits; result is INT64 if x is INT64, else INT32; the operation is defined for n >= 0 and n < SIZE(x)*8; no overflow checks (may halt program)

|BAND(x,y) 
|x, y: integer 
|integer 
|bitwise AND; result is UINT64 if any is UINT64, else INT64 if any is INT64, else UINT32 if any is UINT32, else INT32

|BNOT(x) 
|x: integer 
|integer 
|bitwise NOT; result type corresponds to the type of x

|BOR(x,y) 
|x, y: integer 
|integer 
|bitwise OR; result is UINT64 if any is UINT64, else INT64 if any is INT64, else UINT32 if any is UINT32, else INT32
...
|x, y: integer 
|integer
|bitwise XOR; result is UINT64 if any is UINT64, else INT64 if any is INT64, else UINT32 if any is UINT32, else INT32
...
|UINT32 or UINT64 
|logical shift left by n bits, where n >= 0 and n < SIZE(x)*8; result is UINT64 if x is UINT64 or INT64, else UINT32

|SHR(x,n) 
|x: integer, n: unsigned integer 
|UINT32 or UINT64 
|logical shift right by n bits; result is UINT64 if x is UINT64 or INT64, else UINT32; the operation is defined for n >= 0 and n < SIZE(x)*8; no overflow checks (may halt program)
