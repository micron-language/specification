...
A designator of the form `v(T)` denotes the pointer `v` treated as pointing to a value 
of type `T`. If `T` is a pointer type `POINTER TO T0`, where `T0` is an object type, 
the designator stands for `v` treated as type `T` (referencing `T0`).

If the static type of `v` is a pointer to object type, `T` (or `T0`) must also denote an object type, 
and the designator constitutes a *type guard*. It asserts that the dynamic type of the object 
referenced by `v` extends `T` (or `T0`). If `v` is `NIL` or the assertion fails, program execution aborts.

If neither the static type of `v` nor `T` refer to an object type, the designator implies a static 
reinterpretation of the address `v` as the address of a value of type `T`; no run-time check is performed. 
Other combinations are invalid (use the CAST predeclared function instead, see <<Predeclared function procedures>>).
...
|
|T,x: pointer types; T is a type name
|T 
