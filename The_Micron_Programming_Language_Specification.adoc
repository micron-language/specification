...
zero) must fit into the char array, or it determines its size in case of 
an open array.

If NamedType is an array of byte, it can be initialized
with a single hex string literal. The literal must exactly fit 
the size of the byte array, or it determines its size in case of 
an open array.
